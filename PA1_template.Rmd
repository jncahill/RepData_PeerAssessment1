...
```
<span style="color:blue">The mean of the total number of steps taken per day is **`r as.integer(meanSteps)`.**  
...
```
<span style="color:blue">The 5 minute interval that contains the maximum number of steps is **`r maxInterval`**</span>
...
```
<span style="color:blue">The total number of missing values in the datset is **`r total_NAs`**</span>
...
``` {r Histogram(imputed dataset), echo = TRUE}
...
<span style="color:blue">The mean total number of steps taken per day is **`r mean_Steps_new`**    
The median total number of steps taken per day is **`r median_Steps_new`**</span>  
...
<span style="color:blue">The mean is **`r abs(mean_diff)` steps `r meanPosNeg`** the estimate from the first part of assignment.  
The median is **`r abs(median_diff)` steps `r medianPosNeg`** the estimate from the first part of assignment.**</span>
...
```
There are **`r nrow(activityNA)`** days for which values are imputed.  
